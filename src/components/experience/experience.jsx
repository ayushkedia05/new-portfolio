    ...
    const getCompanyInfo = (companydetails) => {
        return (
            <div className=' flex flex-col max-w-screen-md' >
                ...
                <span className={styles.location}>{jobDetails.location[companydetails]}</span>
                <span className={styles.period}>{jobDetails.period[companydetails]}</span>
        ...
        <div className=' bg-slate-200 p-8 pb-[3rem]'>
            ...
            <div className='flex-col lg:flex justify-around max-w-[70%] m-auto pt-[90px] '>
                <div className='flex-row justify-between lg:flex-col '>
                    <span className="text-4xl font-bold text-center  ">Work Experience</span>


                    <div className=' flex-col text-lg pt-10'>
                        <span className={`${styles.companyName} ${company === "Vaidhyamegha" ? styles.companySelected : undefined
                            } pt-7`} onClick={() => selectedcompany("Vaidhyamegha")}>
                            Vaidhyamegha
                            {company === "Vaidhyamegha" && <BsFillArrowRightCircleFill size={20}></BsFillArrowRightCircleFill>}

                        </span>
                        <span className={`${styles.companyName} ${company === "incribo" ? styles.companySelected : undefined
                            }`} onClick={() => selectedcompany("incribo")}>
                            Incribo.INC
                            {company === "incribo" && <BsFillArrowRightCircleFill size={20}></BsFillArrowRightCircleFill>}


                        </span>
                    </div>
                </div>
                <div className='flex items-center pt-[5rem]'>{getCompanyInfo(company)}</div>
